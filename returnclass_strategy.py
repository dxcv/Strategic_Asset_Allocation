...
def max_sharpe(weights,Returns):
    ...
    #SR = SharpRatio(RET)
    SR = ffn.calc_sharpe(RET)
    #print(SR)
    return -SR
def max_sortino(weights,Returns):
    weights = np.array(weights)
    RET = np.sum(Returns * weights, axis=1)
    sortino =ffn.calc_sortino_ratio(RET)#+10000000)
    return -sortino
def maxR2MDD(weights, Returns):
    ...
    R2MDDadj = (np.average(RET) * 252)/(MDD)#+10000000)
    return -R2MDDadj
...
def maxR2VaR(weights,Returns,method='historical',
             alpha=0.99,
             window = 250,eta=0.99):#99%置信水平，历史收益率法计算VaR
    weights = np.array(weights)
    RET = np.sum(Returns *weights,axis=1)
...
def maxR2CVaR(weights,Returns,alpha=0.99):#99%置信水平，历史收益率法计算VaR
    weights = np.array(weights)
    RET = np.sum(Returns *weights,axis=1)
    sorted_Returns = np.sort(RET)
    index = int(alpha * len(sorted_Returns))
