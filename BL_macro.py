...
def similarity(df, n, weight):
    ...
    first_date = df.index[0]  # 第一条数据对应的日期
    correlation = pd.Series(name='correlation')
    for i in range(len(df) - 2*n):
        temp_corr = []
        start = first_date + i * MonthEnd()
        end = start + (n - 1) * MonthEnd()
        temp = df.loc[start:end, :].copy()  # 每个月都往后推12个月，得到的矩阵与需预测的矩阵比较相关性
        for j in range(len(df.columns)):
            corr = stats.pearsonr(temp.iloc[:,j].values,data_now.iloc[:,j].values)  # 对两个矩阵的每一列分别计算相关系数
    ...
    correlation.sort_values(ascending=False, inplace=True)  # 按照相关系数的大小降序排列
    #correlation.index = pd.DatetimeIndex(correlation.index)
    return correlation

def compute_parameter(m, n, t, theta, rf, used_equity_data, used_macro_data, t_month_return):
    equities = used_equity_data.columns  # 有哪些可供配置的资产
    k = len(equities) # 有多少个可供配置的资产
    mkt_return = used_equity_data.apply(lambda x: x.mean(), axis=1)  # 等权重基准组合的收益
    delta = abs((mkt_return.mean()-rf)/(mkt_return.std())**2)  # 风险厌恶系数
    prior_cov = used_equity_data.cov()  # k*k维的先验协方差矩阵
    P = np.identity(k)  # k*k维的观点矩阵
    ...
    # 取出与所考察时间段最相似的m个历史时间段的结束日期的后t个月
    similar_data = t_month_return.loc[head_m, :]  # 取出最相似的m个时间段的后t个月的资产涨跌情况
    mean_return = similar_data.mean()
    Q = mean_return.values.T  # k*1维的观点收益矩阵
    point_variance = np.power(similar_data.std(),2)
    Omega = np.diag(point_variance.values)  # k*k维的观点误差矩阵
