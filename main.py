...
class Para:
    ...
    wmin = 0
    wmax =1
    m = 8 #用来判断当前资产走势的历史时间段的个数
    theta = 24 #宏观经济变量HP滤波的参数
    rf = 0.025 #如果需要用到无风险收益率
    RiPmethod = 'shrinkage_DW'
    #可输入选项：variance #downward #ledoit_wolf #shrinkage_simple #shrinkage_Ledoit #shrinkage_DW
    RePmethod = 'minMDD'
    #可输入选项：#maxSharp（报错）#max_sortino（报错）#maxR2MDD（报错）#minMDD#maxR2VaR#maxR2CVaR
    VaRmethod='historical'
    #可输入选项：#weighted_historical#RM#EVT#norm#t#CF
para = Para()
...
if __name__ == "__main__":
    ...
    weights_nob1,result_nob1 = MAC_RP.withoutboundary(datas,period=para.period, rollingtime=para.rollingtime,
                                                      method='MAC',tau=para.tau, wmin=para.wmin,wmax=para.wmax)
    ...
    weights_nob2,result_nob2 = MAC_RP.withoutboundary(datas,period=para.period, rollingtime=para.rollingtime,
                                                      method='RP',tau=para.tau,wmin=para.wmin,wmax=para.wmax)
